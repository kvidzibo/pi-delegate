...
The returned answer joins all text blocks from the last assistant message. `maxOutputBytes` caps it with a truncation notice. Natural completion shortened only by this cap remains successful. Completed jobs retain capped results but release process/control references.
