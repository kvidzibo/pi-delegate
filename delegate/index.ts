...
export default function delegate(pi: ExtensionAPI, childRunner: typeof runChild = runChild) {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"On a nonterminal delegate check-in, wait again if progress is useful; otherwise request wrap:true, then cancel:true if still stuck.",
			"delegate wrap:true steers the child to finish; the current tool may complete. cancel:true kills.",
