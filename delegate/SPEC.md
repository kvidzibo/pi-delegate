...
Foreground `timeoutMs` expiry auto-promotes the job to background, detaches the spawn wait's signal, and returns a short nonterminal check-in (`terminal: false`, `jobId`, last tools, `quietForMs`). No thinking tail. Slot remains held. Parent continues with `jobId`, `wrap`, or `cancel`.
...
Observe the current parent run's `ctx.signal` at `agent_start`, replacing earlier subscriptions and detaching at `agent_settled` or shutdown. A parent interrupt (Esc or RPC abort during the run) cancels all outstanding children, including background/promoted jobs from earlier turns. Suppress all existing completion notices before cancellation, including held terminal notices, so they cannot restart the parent. Batch cancellation must not dispatch queued jobs or close the scheduler to later turns. Normal parent completion, wait expiry and menu dismissal do not cancel background work.
...
Collapsed final cards show up to three rendered Markdown lines of the answer/error, except cancellation output is expanded-only. Failed/cancelled rows show the latest recorded tool when available. Expand shows the full returned answer, task, last three tools and archive path for complete recorded history. Recording warnings stay visible. Host/validation errors without details fall back to result content. A `tool_result` hook marks `details.ok: false` as `isError: true` (Pi ignores `isError` returned directly by execute).

Collect/wait/peek/wrap/cancel rows are compact historical receipts with role, full model ID (once), job ID and operation/outcome. Terminal receipts say result/failure collected and include the failure stop reason; aborted jobs say cancelled rather than failure collected. Pending returned receipts explicitly say status was observed at the check. Failed/cancelled receipts add a one-line task and latest recorded tool. Other errors retain their collapsed explanation; raw cancellation output, including internal empty-response placeholders, is expanded-only. Expanded receipts show the task, full result, recent tools and session path without changing the underlying collected evidence.
...
Scheduler tests: local jobs never overlap at `maxLocalConcurrent: 1`; hosted is not blocked by the GPU queue; fg wait budget includes queue time; quiet wait does not kill; wrap steers / queued wrap cancels; promoteBackground detaches the spawn wait's signal; parent-run interruption still cancels all outstanding jobs without completion notices; hard timeout optional; shutdown drops queue.
